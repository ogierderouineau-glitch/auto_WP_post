...
from backend.config import get_active_client_config
...
def taxonomy_terms_endpoint(taxonomy: str) -> str:
    definitions = request_json("GET", "/wp-json/wp/v2/taxonomies")
    definition = definitions.get(taxonomy) if isinstance(definitions, dict) else None
    if definition is None and isinstance(definitions, dict):
        definition = next(
            (
                item
                ...
                and taxonomy in {
                    ...
                    str(item.get("rest_base") or "").strip(),
                }
            ),
            None,
        )
    if not isinstance(definition, dict):
        raise ValueError(
            f"WordPress REST does not expose taxonomy {taxonomy!r} in its taxonomy registry."
        )
    rest_base = str(definition.get("rest_base") or taxonomy).strip("/")
    rest_namespace = str(definition.get("rest_namespace") or "wp/v2").strip("/")
    return f"/wp-json/{rest_namespace}/{rest_base}"


def _term_collection_path(endpoint: str) -> str:
    return endpoint if endpoint.startswith("/") else f"/wp-json/wp/v2/{endpoint}"


def find_term(endpoint: str, name: str) -> dict[str, Any] | None:
    terms = request_json(
        "GET",
        _term_collection_path(endpoint),
...
def create_term(endpoint: str, name: str) -> dict[str, Any]:
    return request_json("POST", _term_collection_path(endpoint), json={"name": name})


def registered_terms(endpoint: str) -> list[dict[str, Any]]:
    ...
    while True:
        ...
        if not isinstance(batch, list):
            raise ValueError(f"WordPress returned invalid terms for {endpoint!r}.")
        terms.extend(item for item in batch if isinstance(item, dict))
        if len(batch) < 100:
            return terms
        page += 1
...
def match_registered_term(
    ...
    *,
    ...
    requested_key = normalized_term_key(requested)
    exact = [
        term
        for term in terms
        if requested_key
        and requested_key in {
            normalized_term_key(term.get("name")),
            normalized_term_key(term.get("slug")),
        }
    ]
    if len(exact) == 1:
        return exact[0]
    if len(exact) > 1 or len(requested_key) < 5:
        return None
