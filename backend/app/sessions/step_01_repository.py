...
from backend.app.errors import SessionNotFoundError, SessionVersionConflictError
...
class FileSessionRepository(SessionRepository):
    ...
    def save(self, session: ContentSession, *, expected_version: int) -> ContentSession:
        ...
        self._write(self._resolve_path(session.session_id), updated)
        return updated

    def list(self) -> list[ContentSession]:
        sessions: list[ContentSession] = []
        seen_paths: set[Path] = set()
        for base_root in [self.root, *self.extra_roots]:
            if not base_root.exists():
                continue
            for path in base_root.rglob("state.json"):
                if not path.is_file():
                    continue
    ...
    def _resolve_path(self, session_id: str) -> Path:
        ...
        for base_root in [self.root, *self.extra_roots]:
            candidate = base_root / safe_id / "state.json"
            if candidate.is_file():
                return candidate
        return self.root / safe_id / "state.json"

    def _path(self, session_id: str) -> Path:
        return self.root / Path(session_id).name / "state.json"
