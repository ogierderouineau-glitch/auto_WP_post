...
def build_image_metadata_batch_model(
    ...
    *,
    media_ids: tuple[str, ...],
    taxonomy_candidates: dict[str, tuple[str, ...]] | None = None,
) -> type[BaseModel]:
    metadata_model = build_image_metadata_model(rows, enum_families={})
    taxonomy_fields: dict[str, tuple[Any, Field]] = {}
    for taxonomy, terms in (taxonomy_candidates or {}).items():
        if terms:
            taxonomy_fields[taxonomy] = (
                list[Literal.__getitem__(terms)],
                Field(
                    ...,
                    description=(
                        "Select only exact terms whose subject is visibly present "
                        "in this image; otherwise return an empty list."
                    ),
                ),
            )
    visible_taxonomies_model = create_model(
    ...
    if taxonomy_fields:
        item_fields["visible_taxonomy_terms"] = (visible_taxonomies_model, ...)
