...
from backend.app.api.step_01_models import (
    AnswersRequest,
    AnalyzeRequest,
    ApproveRequest,
...
from backend.config import V2_LANGUAGE_MODEL, get_active_client_id
from backend.app.api.step_03_container import client_knowledge_gcs_uri
...
from backend.app.storage.step_02_uploads import safe_upload_name, validate_upload


_SESSION_JOBS: dict[str, dict[str, Any]] = {}
_LOGGER = logging.getLogger("speech2post")
...
def create_router(
    ...
    async def create_session(
        ...
        values["user_id"] = get_active_client_id()
        return SessionResponse(session=service_provider().create(**values))
    ...
    async def workbook_status(
        ...
        return {
            **snapshot.version.model_dump(mode="json"),
            "storage_mode": "gcs" if client_knowledge_gcs_uri() else "local_file",
            "knowledge_source_policy": "gcs_required" if client_knowledge_gcs_uri() else "local_only",
            "gcs_uri": client_knowledge_gcs_uri() or None,
    ...
    async def get_session(
        ...
        service_provider().require_owner(session_id, get_active_client_id())
    ...
    async def add_inputs(
        ...
        service = service_provider()
        service.require_owner(session_id, get_active_client_id())
    ...
    async def start_generate_job(
        ...
        _SESSION_JOB_EXECUTOR.submit(
            _run_session_job,
            job["job_id"],
            _fixed_service_provider(service_provider()),
            session_id,
            "generate",
            payload.model_dump(),
        )
    ...
    async def session_job(job_id: str) -> dict[str, Any]:
        ...
        if job.get("client_id") != get_active_client_id():
...
def _fixed_service_provider(
    service: ContentSessionService,
) -> Callable[[], ContentSessionService]:
    """Keep background work bound to the authenticated client's service."""
