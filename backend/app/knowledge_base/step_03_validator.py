...
from backend.app.images.step_01_conditions import IMAGE_CONDITION_HANDLERS
from backend.app.payloads.step_01_transforms import TRANSFORMS
...
class WorkbookValidator:
    ...
    def validate(self, snapshot: WorkbookSnapshot) -> WorkbookSnapshot:
        ...
        self._unique_taxonomies(errors, snapshot)
        ...
        for taxonomy in snapshot.taxonomies:
            if (
                taxonomy.post_type_key != "*"
                and taxonomy.post_type_key not in post_types
            ):
                errors.append(self._error(
                    ...
                    "post_type_key",
                    "unknown_post_type",
                    f"Post type {taxonomy.post_type_key!r} is not defined on post_types.",
                ))
        ...
        for row in snapshot.post_types:
            ...
            for wp_taxonomy in row.wp_taxonomies:
                taxonomy = (
                    taxonomies.get((row.post_type_key, wp_taxonomy))
                    or taxonomies.get(("*", wp_taxonomy))
                )
                if wp_taxonomy in seen_taxonomies:
                    errors.append(self._error(
                        "post_types",
                        row.sheet_row,
                        "wp_taxonomies",
                        "duplicate_wordpress_taxonomy",
                        f"WordPress taxonomy {wp_taxonomy!r} is selected more than once.",
                    ))
                    continue
                seen_taxonomies.add(wp_taxonomy)
                if taxonomy is None:
                    errors.append(self._error(
                        "post_types",
                        row.sheet_row,
                        "wp_taxonomies",
                        "unknown_wordpress_taxonomy",
                        f"WordPress taxonomy {wp_taxonomy!r} is not defined for "
                        f"post type {row.post_type_key!r} on the taxonomies tab.",
                    ))
                    continue
                source_kind, separator, source_value = taxonomy.taxonomy_term_source.partition(":")
                valid_source = (
                ...
                if not valid_source:
                    errors.append(self._error(
                        ...
                        f"acf:<field_key>, or fact:<input_fact_key> for post type {row.post_type_key!r}.",
                    ))
    ...
    @staticmethod
    def _unique_taxonomies(
        errors: list[ErrorDetail],
        snapshot: WorkbookSnapshot,
    ) -> None:
        seen: set[tuple[str, str]] = set()
        for row in snapshot.taxonomies:
            relationship = (row.post_type_key, row.wp_taxonomy)
            if relationship in seen:
                errors.append(ErrorDetail(
                    sheet="taxonomies",
                    row=row.sheet_row,
                    column="wp_taxonomy",
                    error_code="duplicate_taxonomy_relationship",
                    message=(
                        f"Taxonomy {row.wp_taxonomy!r} is configured more than once "
                        f"for post type {row.post_type_key!r}."
                    ),
                ))
