...
class PostTypeConfig(WorkbookRow):
    ...
    @field_validator("wp_taxonomies", mode="before")
    @classmethod
    def normalize_taxonomy_slugs(cls, value: Any) -> tuple[str, ...]:
...
class TaxonomyConfig(WorkbookRow):
    post_type_key: str = "*"
    wp_taxonomy: str
    taxonomy_term_source: str
    # Retained for backwards compatibility with the first taxonomies-tab format.
    assign_taxonomy_to_media: bool = False

    @field_validator("wp_taxonomy", mode="before")
    @classmethod
    def normalize_taxonomy_slug(cls, value: Any) -> str:
        return str(value or "").strip().lower()

    @field_validator("taxonomy_term_source", mode="before")
    @classmethod
    def normalize_term_source(cls, value: Any) -> str:
...
class WorkbookSnapshot(BaseModel):
    ...
    taxonomies: tuple[TaxonomyConfig, ...]
