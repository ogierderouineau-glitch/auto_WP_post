...
from backend.config import DEFAULT_CLIENT_ID, set_active_client
...
class ApiTests(unittest.IsolatedAsyncioTestCase):
    ...
    async def asyncTearDown(self) -> None:
        ...
        set_active_client(DEFAULT_CLIENT_ID)
    ...
    async def test_workbook_status_exposes_version_hash(self) -> None:
        ...
        if response.json()["acf_fields"]:
            self.assertIn("value_type", response.json()["acf_fields"][0])
    ...
    async def test_user_header_cannot_change_session_owner(self) -> None:
        created = (await self.client.post(
            "/api/content-sessions",
            json={"user_id": "user-1", "post_type_key": "event"},
        )).json()["session"]
        ...
        self.assertEqual(response.status_code, 200)
        self.assertEqual(response.json()["session"]["user_id"], "user-1")

    async def test_create_uses_authenticated_client_not_submitted_identity(self) -> None:
        response = await self.client.post(
            "/api/content-sessions",
            headers={"X-User-ID": "other-user"},
            json={"user_id": "other-user", "post_type_key": "event"},
        )
