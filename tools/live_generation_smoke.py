...
from backend.app.providers.step_02_openai import OpenAILanguageModelProvider
...
from backend.config import OPENAI_API_KEY, V2_LANGUAGE_MODEL

SYNTHETIC_EVENT = """
Das Eventteam betreut am 24.06.2026 ein Firmen-Sommerfest für 120 Gäste bei
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Run a live OpenAI generation smoke test without WordPress publication."
    )
    parser.add_argument(
        "--workbook",
        type=Path,
        default=Path("data/knowledge/default.xlsm"),
    )
    parser.add_argument(
        "--output",
        type=Path,
        default=Path("data/audits/live_generation_smoke.json"),
    )
