...
export function AuthModal({
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    const cleanedUser = (userId || "flairlab").trim()
    if (!cleanedKey) {
    ...
    setSuccess("")
    try {
      const data = await validateImportKey({ apiKey: cleanedKey, userId: cleanedUser })
      const client = data.clients.find((item) => item.client_id === cleanedUser) ?? data.clients[0]
