...
def test_final_draft_validation_uses_the_same_minimum_tolerance() -> None:
    ...
    DraftValidator._limits(
        accepted_errors, "sheet", 1, "hero_intro", "word " * 56, 70, 80, None, None,
    )
    assert accepted_errors == []

    rejected_errors = []
    DraftValidator._limits(
        rejected_errors, "sheet", 1, "hero_intro", "word " * 55, 70, 80, None, None,
    )
    assert rejected_errors[0].error_code == "minimum_words_not_met"


def test_final_draft_validation_uses_the_same_maximum_tolerance() -> None:
    accepted_errors = []
    DraftValidator._limits(
        accepted_errors, "sheet", 1, "hero_intro", "word " * 96, 70, 80, None, None,
    )
    assert accepted_errors == []

    rejected_errors = []
    DraftValidator._limits(
        rejected_errors, "sheet", 1, "hero_intro", "word " * 97, 70, 80, None, None,
    )
    assert rejected_errors[0].error_code == "maximum_words_exceeded"
