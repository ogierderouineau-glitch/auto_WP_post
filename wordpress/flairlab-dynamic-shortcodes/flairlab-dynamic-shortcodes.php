<?php
/**
 * Plugin Name: FLAIRLAB Dynamic Shortcodes
...
function flairlab_sanitize_generated_variables($value): array {
    if (!is_array($value)) {
        return [];
    }
 ...
 */
function flairlab_register_generated_variables_meta(): void {
    $post_types = get_post_types(['show_in_rest' => true], 'names');
    $post_types = apply_filters(
        'flairlab_generated_variable_post_types',
        $post_types
    );
    ...
    foreach (array_unique((array) $post_types) as $post_type) {
        register_post_meta(
            ...
            [
                ...
                'sanitize_callback' => 'flairlab_sanitize_generated_variables',
...
add_action('init', 'flairlab_register_generated_variables_meta', 100);
 ...
 */
function flairlab_post_variable_shortcode($attributes = []): string {
...
add_shortcode('post_variable', 'flairlab_post_variable_shortcode');
